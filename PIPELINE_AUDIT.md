...
2. [Fix Specifications (8 fixes, dependency-ordered)](#fix-specifications)
...
Columns not in REGISTRY that SEED needs:
    module                   →  hardcode "A"
    relation_label           →  derive from mechanism_description (first 80 chars)
...
- `confidence_note` must document the conversion: "Converted from partial η²=0.35 using d = 2√(η²/(1−η²))"
...
```
FIX-1 (edge unification)
  └─→ FIX-2 (persistence guard)         [no dependency on 1, but do together]
        └─→ FIX-3 (expanded template)
              └─→ FIX-7 (convert Campbell effects)
              └─→ FIX-8 (fill missing templates)
                    └─→ FIX-4 (unified writer with validation)
...
- FIX-3 defines the expanded template that FIX-7/8 will fill
- FIX-7/8 fix the actual data before FIX-4 validates it
- FIX-4 must exist before FIX-5 can call it
- FIX-6 runs last to verify everything
...
```sql
-- No UNASSIGNED edge IDs
SELECT COUNT(*) FROM edge_evidence_v1 WHERE edge_relation_id = 'UNASSIGNED';
-- Expected: 0
...
-- All evidence rows have study_design in valid enum
SELECT COUNT(*) FROM edge_evidence_v1 
WHERE effect_type_reported NOT IN ('cohen_d', 'cohen_d_from_eta_sq', 'log_or', 'harmonized_beta');
-- Expected: 0 (all should be standardized)

-- Critical columns populated for manual entries
SELECT COUNT(*) FROM edge_evidence_v1 
